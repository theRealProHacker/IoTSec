...
\caption{Lexing and parsing of the expression \texttt{(3.5+7)}. The lexer
(Python's \texttt{tokenize}) splits the source into a token stream; the parser
(\texttt{ast.parse}) assembles it into an abstract syntax tree. }
\label{fig:lex-parse}
\end{figure}
